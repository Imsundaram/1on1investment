...
export async function updateSocials(formData: FormData) {
    const fields = [
        ...
        "heroImage", "whyUsImage", "aboutImage", "logoUrl"
    ];
...
export async function submitInquiry(formData: FormData) {
    ...
    if (USE_MONGO) {
        try {
            ...
            // Return more specific error details to help the user debug
            if (error.name === 'MongooseServerSelectionError') {
                return { success: false, error: "Database connection failed. Please ensure IP 0.0.0.0/0 is whitelisted in MongoDB Atlas Network Access." };
            }
            if (error.message?.includes('Authentication failed')) {
                return { success: false, error: "Database authentication failed. Please check your username and password in the Vercel MONGODB_URI variable." };
            }
            return { success: false, error: `Database Error: ${error.name} - ${error.message}` };
        }
    }
